...
IDEAS_PER_TYPE_VALUES = sorted([1])
TYPES_PER_FACT_VALUES = sorted([1, 5, 10])
...
log().add_to_log_dict(
    sweep_id=sweep_id, ideas_per_type_values=IDEAS_PER_TYPE_VALUES, total_epochs_at_max_ideas=TOTAL_EPOCHS_AT_MAX_IDEAS
)
args_list = []

for ideas_per_type in IDEAS_PER_TYPE_VALUES:
    for types_per_fact in TYPES_PER_FACT_VALUES:
        ...
        python_args = [
            ...
            "None",
            "--experiment_name",
            f"first_time_generating_synthetic_ideas{ideas_per_type}_epochs{epochs_value}",
            "--fact_dataset_type",
            "synthetic_docs",
            "--float_type",
            "bf16",
