...
def main(args: TrainingArgsSlurm):
    ...
    if len(arguments) != args.slurm_array_max_ind + 1:
        raise ValueError(
            f"Slurm array should be the same size as the number of arguments, but is {args.slurm_array_max_ind + 1} and there are {len(arguments)} arguments"
        )
    ...
    create_symlinks_for_slurm_output(
        args
    )  # This should go after the arguments are set, as it uses them
