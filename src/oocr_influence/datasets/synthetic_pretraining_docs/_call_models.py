...
class Universe(BaseModel):
    ...
    def merge_facts_from(self, other: "Universe", on: str) -> "Universe":
        ...
        for f_self, f_other in zip(f_self_sorted, f_other_sorted):
            ...
            assert len(set(fields_self.keys()) & set(fields_other.keys())) == 0, f"Overlapping fields: {set(fields_self.keys()) & set(fields_other.keys())}"
            merged_feature_sets.append(FeatureSet(id=f"{f_self.id}_merged_{f_other.id}", fields={**fields_self, **fields_other}))
        return Universe(
...
async def generate_document(
    ...
    if generation_instructions is None:
        ...
        generation_instructions = generation_instructions.format(**doc_spec.fact.feature_set.fields)
