...
class InfluenceArgs(BaseModel):
    ...
    use_flash_attn: bool = False  # TODO: CHange once instlal sues are fixed
...
def main(args: InfluenceArgs):
    ...
    assert isinstance(model, GPT2LMHeadModel) or isinstance(model, OlmoForCausalLM) or isinstance(model, LlamaForCausalLM), (
    ...
    with prepare_model_for_influence(model=model, task=task):
        ...
        logger.info(f"Computing influence scores for {args.experiment_name} and {args.query_name_extra}")
        influence_scores, scores_save_path = get_pairwise_influence_scores(  # type: ignore
            ...
            query_indices=args.query_dataset_indices,
            task=task,
...
def get_datasets(args: InfluenceArgs) -> tuple[Dataset, Dataset]:
    _, train_dataset, query_dataset, _, _ = load_experiment_checkpoint(
