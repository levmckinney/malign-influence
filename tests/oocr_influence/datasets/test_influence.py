...
def test_prepare_model_for_influence_conv1d_replacement():
    ...
    with prepare_model_for_influence(model=model, task=task) as prepared_model:
        ...
        assert linear_count_during > linear_count_before, (
            "Expected more Linear modules after replacement"
        )

        # Check that parameters are not trainable during preparation
        for name, param in prepared_model.named_parameters():
            if (
                "_constant" not in name
            ):  # Ignore constant parameters, added by wrap_tracked_modules
                assert not param.requires_grad, (
                    "Expected all parameters to be non-trainable during preparation"
                )
    ...
    assert conv1d_count_after == conv1d_count_before, (
        "Expected Conv1D modules to be restored"
    )
    assert linear_count_after == linear_count_before, (
        "Expected Linear modules to be restored to original count"
    )
...
def test_prepare_model_for_influence_tracked_module_installation():
    ...
    assert supported_modules_before > 0, (
        "Expected GPT-2 model to contain supported modules"
    )
    assert tracked_modules_before == 0, (
        "Expected no TrackedModule instances before preparation"
    )
    ...
    with prepare_model_for_influence(model=model, task=task) as prepared_model:
        ...
        for module in prepared_model.modules():
            if isinstance(
                module, tuple(TrackedModule.SUPPORTED_MODULES)
            ) and not isinstance(module, TrackedModule):
                supported_modules_during += 1
            if isinstance(module, TrackedModule):
                tracked_modules_during += 1

        # Verify modules were wrapped with TrackedModule
        assert tracked_modules_during > 0, (
            "Expected TrackedModule instances to be installed"
        )
        ...
        assert tracked_modules_during > 0, (
            "Expected some modules to be wrapped with TrackedModule"
        )
    ...
    # Verify the model was restored to its original state
    assert supported_modules_after == supported_modules_before, (
        "Expected supported modules to be restored"
    )
    assert tracked_modules_after == 0, (
        "Expected no TrackedModule instances after restoration"
    )
